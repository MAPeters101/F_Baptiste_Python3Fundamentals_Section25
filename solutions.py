...
We can see the CSV format is the standard one, and we have three columns with
the headers in the first row.

We could write a function that loads the entire data set into memory and then
processes it (adding the amount), but that's inefficient - we could just read
the file row by row and keep a cumulative sum of the amount field.

Also, instead of writing two separate functions, one to cast the amount value
to a float andthe other to cast it to a Decimal, we'll implement it as a
single function and pass to the function an argument indicating whether we
want to work with floats, or with Decimals.
...
As you can see, cumulative representation errors, even with two million
transactions, is quite close to the exact value we obtained using Decimal
objects.
...
time_float
8.843322478
time_decimal = timeit('sum_amount(f_name, as_decimal=True)',
...
16.2
So using Decimal was slower than using floats (and of course as the number of
transactions increase we can expect this value to grow larger), and the loss
of precision was likely acceptable in this particular scenario.
...
Solution
Here we'll take the same approach as before - we'll load the file and process
rows one by one, keeping a running total of the commissions.
...
def sum_fees(f_name, fee_perc='0.00123'):
    ...
    return total
Now, we still have to restrict our fee calculation to 8 digits after the
decimal point, and we do not want to use Banker's rounding - instead we need
to round ties away from zero always.

Note: we have to round after we calculate the absolute value, otherwise we'll
round in the wrong direction for fees on negative amounts.

For Decimal objects changing the rounding is easy, we can use a context to set
the rounding mechanism - in this case we'll want to use ROUND_HALF_UP.

Since we want to compare the two rounding methods, we'll pass that in as an
argument as well.

import decimal

def sum_fees(f_name, fee_perc='0.00123', *, round_method=decimal.ROUND_HALF_UP,
ndigits=8):
...
fees_round_half_even = sum_fees('transactions.csv',
round_method=decimal.ROUND_HALF_EVEN)
